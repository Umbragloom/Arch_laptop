...
import { tslib, tsp, KindModifiers, toSymbolDisplayPartKind } from './ts-protocol.js';
...
var ScriptElementKind = tslib.ScriptElementKind;
var SymbolDisplayPartKind = tslib.SymbolDisplayPartKind;
export function asCompletionItem(entry, optionalReplacementSpan, file, position, document, filePathConverter, options, features) {
    const item = {
        label: entry.name,
...
export async function asResolvedCompletionItem(item, details, client, filePathConverter, options, features) {
...
function asDetail({ displayParts, sourceDisplay, source: deprecatedSource }, filePathConverter) {
    const result = [];
...
export function getCompletionTriggerCharacter(character) {
    switch (character) {
        case '@':
        case '#':
        case ' ':
